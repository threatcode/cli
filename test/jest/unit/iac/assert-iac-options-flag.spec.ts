...
describe('assertIntegratedIaCOnlyOptions()', () => {
  ...
  it('Refuses threatcode-cloud-environment flag', () => {
    const options = ['--threatcode-cloud-environment', 'envid'];
    expect(() =>
      assertIntegratedIaCOnlyOptions(org, [...command, ...options, ...files]),
    ).toThrow(new IntegratedFlagError('threatcode-cloud-environment', org.meta.org));
  });
});
