...
import * as threatcodePolicyLib from 'threatcode-policy';

describe('CLI Share Results', () => {
  let threatcodePolicy: Policy;
  ...
  beforeAll(async () => {
    threatcodePolicy = await threatcodePolicyLib.load('test/jest/unit/iac/fixtures');
  });
  ...
  it("converts the results to Envelope's ScanResult interface - without .threatcode policies", async () => {
    await shareResults({
      results: scanResults,
      policy: undefined,
      meta: {
        ...
        orgName: 'org-name',
        gitRemoteUrl: 'http://github.com/threatcode/cli.git',
      },
      options: {},
    });
  ...
  it("converts the results to Envelope's ScanResult interface - with .threatcode policies", async () => {
    await shareResults({
      results: scanResults,
      policy: threatcodePolicy,
      meta: {
        projectName: 'project-name',
        orgName: 'org-name',
        gitRemoteUrl: 'http://github.com/threatcode/cli.git',
