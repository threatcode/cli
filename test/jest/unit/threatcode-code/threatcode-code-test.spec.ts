...
import { analyzeFolders, AnalysisSeverity } from '@threatcode/code-client';
jest.mock('@threatcode/code-client');
...
import threatcodeTest from '../../../../src/cli/commands/test/';
...
describe('Test threatcode code', () => {
  ...
  afterEach(() => {
    delete process.env.THREATCODE_OAUTH_TOKEN;
    jest.resetAllMocks();
  });
  ...
  it('should use oauth token for auth if provided', async () => {
    ...
    process.env.THREATCODE_OAUTH_TOKEN = oauthToken;
  ...
  it('should succeed testing from the cli test command - with correct exit code', async () => {
    ...
    expect.hasAssertions();
    try {
      await threatcodeTest('some/path', options);
    } catch (error) {
      const errMessage = stripAscii(stripAnsi(error.message.trim()));
      const expectedOutput = stripAscii(stripAnsi(testOutput.trim()));
  ...
  it('should show error if sast is not enabled', async () => {
    ...
    await expect(
      threatcodeTest('some/path', { code: true, _: [], _doubleDashArgs: [] }),
    ).rejects.toHaveProperty(
      'userMessage',
      'Threatcode Code is not supported for org: enable in Settings > Threatcode Code',
    );
  });
  ...
  it('Local code engine - should throw error, when enabled and url is missing', async () => {
    ...
    await expect(
      ...
      'Missing configuration for Threatcode Code Local Engine. Refer to our docs on https://threatcode.github.io/docs/products/threatcode-code/deployment-options/threatcode-code-local-engine/cli-and-ide to learn more',
    );
  });
});
