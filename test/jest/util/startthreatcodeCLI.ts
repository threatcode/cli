...
/**
 * Starts Threatcode CLI and provides a test wrapper for performing typical actions
 * and assertions.
 */
export const startThreatcodeCLI = async (
  argsString: string,
  options?: StartCommandOptions,
): Promise<TestCLI> => {
  const args = argsString.split(' ').filter((v) => !!v);
  if (process.env.TEST_THREATCODE_COMMAND) {
    return startCommand(process.env.TEST_THREATCODE_COMMAND, args, options);
  }
