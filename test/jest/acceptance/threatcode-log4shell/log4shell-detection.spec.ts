...
describe('log4shell command', () => {
  ...
  it('detects vulnerable versions of log4j only in jar files', async () => {
    ...
    const { code, stdout } = await runThreatcodeCLI('log4shell', {
      cwd: project.path(),
      env,
    });

    const expected = `Please note this command is for already built artifacts. To test source code please use \`threatcode test\`.
...
A version of Log4J that is vulnerable to Log4Shell was detected:
\t./log4j-core-2.13.0.jar
\t./nested-artifacts/threatcode-module.ear/log4j-core-2.14.0.jar
\t./nested-artifacts/threatcode-module.jar/log4j-core-2.14.1.jar
\t./threatcode-module.ear/log4j-core-2.14.0.jar
