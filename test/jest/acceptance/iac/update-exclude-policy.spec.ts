...
describe('iac update-exclude-policy', () => {
  ...
  it('update-exclude-policy fail without the right entitlement', async () => {
    const { stdout, stderr, exitCode } = await run(
      `threatcode iac update-exclude-policy --org=no-iac-drift-entitlements`,
      {},
    );

    expect(stdout).toMatch(
      'Command "update-exclude-policy" is currently not supported for this org. To enable it, please contact threatcode support.',
    );
  ...
  it('update-exclude-policy successfully executed when org has the entitlement', async () => {
    ...
    const threatcodeBinaryPath = path.join(__dirname, '../../../../bin/threatcode');

    const { stderr, stdout, exitCode } = await run(
      `cat ${analysisPath} | ${threatcodeBinaryPath} iac update-exclude-policy`,
      {},
      tmpFolderPath,
    );
