...
describe('ARM single file scan', () => {
  ...
  it('finds issues in ARM JSON file', async () => {
    const { stdout, exitCode } = await run(
      `threatcode iac test ./iac/arm/rule_test.json`,
    );
  ...
  it('filters out issues when using severity threshold', async () => {
    const { stdout, exitCode } = await run(
      `threatcode iac test ./iac/arm/rule_test.json --severity-threshold=high`,
    );
  ...
  it('outputs an error for files with no valid JSON', async () => {
    const { stdout, exitCode } = await run(
      `threatcode iac test ./iac/arm/invalid_rule_test.json`,
    );
  ...
  it('outputs the expected text when running with --sarif flag', async () => {
    const { stdout, exitCode } = await run(
      `threatcode iac test ./iac/arm/rule_test.json --sarif`,
    );

    expect(stdout).toContain('"id": "THREATCODE-CC-TF-20",');
    expect(stdout).toContain('"ruleId": "THREATCODE-CC-TF-20",');
    expect(exitCode).toBe(1);
  });

  it('outputs the expected text when running with --json flag', async () => {
    const { stdout, exitCode } = await run(
      `threatcode iac test ./iac/arm/rule_test.json --json`,
    );
