...
describe('iac test --rules', () => {
  ...
  it('scans local custom rules provided via the --rules flag', async () => {
    const { stdout, exitCode } = await run(
      `threatcode iac test --rules=./iac/custom-rules/custom.tar.gz ./iac/terraform/sg_open_ssh.tf`,
    );
  ...
  it('does not show custom rules message if it scans local custom rules and uses --json flag', async () => {
    const { stdout, exitCode } = await run(
      `threatcode iac test --rules=./iac/custom-rules/custom.tar.gz ./iac/terraform/sg_open_ssh.tf --json`,
    );
  ...
  it('presents an error message when the local rules cannot be found', async () => {
    const { stdout, exitCode } = await run(
      `threatcode iac test --rules=./not/a/real/path.tar.gz ./iac/terraform/sg_open_ssh.tf`,
    );
    expect(stdout).toContain(
  ...
  it('presents an error message when the user is not entitled to custom-rules', async () => {
    const { stdout, exitCode } = await run(
      `threatcode iac test --org=no-custom-rules-entitlements --rules=./iac/custom-rules/custom.tar.gz ./iac/terraform/sg_open_ssh.tf`,
    );
    expect(stdout).toContain(
      `Flag "--rules" is currently not supported for this org. To enable it, please contact threatcode support.`,
    );
    expect(exitCode).toBe(2);
  });
  ...
  describe('when used with the --report flag', () => {
    it('should resolve successfully', async () => {
      const { stderr, exitCode } = await run(
        `threatcode iac test --report --rules=./iac/custom-rules/custom.tar.gz ./iac/terraform/sg_open_ssh.tf`,
      );
    ...
    it('should display a warning message for custom rules not being available on the platform if iacShareCliResultsCustomRules feature flag is not enabled', async () => {
      ...
      expect(stdout).toContain(
        "Please note that your custom rules will not be sent to the Threatcode platform, and will not be available on the project's page.",
      );
    });
    ...
    describe.each(['--json', '--sarif'])(
      ...
      (testedFormatFlag) => {
        it('should not display the warning message for the custom rules not being available on the platform', async () => {
          ...
          const { stdout } = await run(
            `threatcode iac test --report --rules=./iac/custom-rules/custom.tar.gz ./iac/terraform/sg_open_ssh.tf ${testedFormatFlag}`,
          );
