...
describe('CLI Share Results', () => {
  ...
  describe('feature flag is not enabled', () => {
    ...
    it('the output includes an error', async () => {
      const { stdout, exitCode } = await run(
        `threatcode iac test ./iac/arm/rule_test.json --report`,
      );

      expect(stdout).toMatch(
        'Flag "--report" is only supported if feature flag "iacCliShareResults" is enabled. To enable it, please contact Threatcode support.',
      );
  ...
  describe('feature flag is enabled', () => {
    ...
    it('forwards project tags', async () => {
      const { exitCode } = await run(
        'threatcode iac test ./iac/arm/rule_test.json --report --tags=foo=bar',
      );
    ...
    it('forwards project environment', async () => {
      const { exitCode } = await run(
        'threatcode iac test ./iac/arm/rule_test.json --report --project-environment=saas',
      );
    ...
    it('forwards project lifecycle', async () => {
      const { exitCode } = await run(
        'threatcode iac test ./iac/arm/rule_test.json --report --project-lifecycle=sandbox',
      );
    ...
    it('forwards project business criticality', async () => {
      const { exitCode } = await run(
        'threatcode iac test ./iac/arm/rule_test.json --report --project-business-criticality=high',
      );
    ...
    it('should print an error message if test usage is exceeded', async () => {
      ...
      const { stdout, exitCode } = await run(
        'threatcode iac test ./iac/arm/rule_test.json --report --project-business-criticality=high --org=aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee',
      );
    ...
    it('should filter out NONE custom policies severity issues and then forward', async () => {
      const { exitCode } = await run(
        'threatcode iac test ./iac/arm/rule_test.json --report --org=custom-policies',
      );
      ...
      // The other THREATCODE-CC-AZURE-543 issue has been filtered out
    ...
    describe('with target reference', () => {
      it('forwards the target reference to iac-cli-share-results endpoint', async () => {
        ...
        const { exitCode } = await run(
          `threatcode iac test ./iac/arm/rule_test.json --report --target-reference=${testTargetRef}`,
        );
