...
describe('iac test text output', () => {
  ...
  it('should show the IaC test title', async () => {
    ...
    const { stdout } = await run(`threatcode iac test ${dirPath}`);

    expect(stdout).toContain('Threatcode Infrastructure as Code');
  });
  ...
  it('should show the spinner message', async () => {
    ...
    expect(stdout).toContain(
      'Threatcode testing Infrastructure as Code configuration issues.',
    );
  });
  ...
  it('should show the issues list section with correct values', async () => {
    const { stdout } = await run('threatcode iac test  ./iac/arm/rule_test.json');
    ...
    expect(stdout).toContain(
      ...
      '           resource' +
      EOL +
      '  Rule:    https://threatcode.github.io/security-rules/THREATCODE-CC-TF-20' +
      EOL +
  ...
  it('should show the test summary section with correct values', async () => {
    ...
    const policyPath = `iac/policy/.threatcode`;
    const { stdout } = await run(
      `threatcode iac test ${dirPath} --policy-path=${policyPath}`,
    );
    expect(stdout).toContain(
      'Test Summary' +
      EOL.repeat(2) +
  ...
  describe('with multiple test results', () => {
    describe('with test failures', () => {
      it('should show the failures list section with the correct values', async () => {
        const dirPath = 'iac';
        const { stdout } = await run(
          `threatcode iac test ${dirPath} my-imaginary-file.tf my-imaginary-directory/`,
        );
  ...
  describe('with only test failures', () => {
    it('should display the test failures list', async () => {
      ...
      const { stdout } = await run(`threatcode iac test ${invalidPaths.join(' ')}`);
  ...
  describe('with no issues', () => {
    it('should display an appropriate message in the issues section', async () => {
      ...
      const { stdout } = await run(`threatcode iac test ${filePath}`);
  ...
  describe('with issues generated by custom rules', () => {
    it('should include the public custom rule IDs', async () => {
      ...
      const { stdout } = await run(
        `threatcode iac test ${filePath} --rules=./iac/custom-rules/custom.tar.gz`,
      );
