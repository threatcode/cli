...
const paths = envPaths('threatcode');
...
describe('iac describe', () => {
  ...
  it('describe fail when not used as an iac sub-command', async () => {
    const { stdout, stderr, exitCode } = await run(`threatcode describe`, {});
  ...
  it('describe fail without the right entitlement', async () => {
    const { stdout, stderr, exitCode } = await run(
      `threatcode iac describe --all --org=no-iac-drift-entitlements`,
      {
        THREATCODE_DRIFTCTL_PATH: './iac/drift/args-echo',
      },
    );

    expect(stdout).toMatch(
      'Command "drift" is currently not supported for this org. To enable it, please contact threatcode support.',
    );
    expect(stderr).toMatch('');
    expect(exitCode).toBe(2);
  });
  ...
  it('Test when driftctl scan exit in error', async () => {
    const { stdout, stderr, exitCode } = await run(`threatcode iac describe  --all`, {
      THREATCODE_FIXTURE_OUTPUT_PATH: outputFile,
      DCTL_EXIT_CODE: '2',
      THREATCODE_DRIFTCTL_PATH: path.join(
        getFixturePath('iac'),
        'drift',
        'args-echo.sh',
      ),
  ...
  it('Launch driftctl from THREATCODE_DRIFTCTL_PATH env var when org has the entitlement', async () => {
    const { stdout, stderr, exitCode } = await run(`threatcode iac describe  --all`, {
      THREATCODE_FIXTURE_OUTPUT_PATH: outputFile,
      THREATCODE_DRIFTCTL_PATH: path.join(
        getFixturePath('iac'),
    ...
    expect(output).toContain('DCTL_IS_THREATCODE=true');
  ...
  it('Launch driftctl from THREATCODE_DRIFTCTL_PATH with html output', async () => {
    const { stdout, stderr, exitCode } = await run(
      `threatcode iac describe  --all --html`,
      {
        THREATCODE_FIXTURE_OUTPUT_PATH: outputFile,
        THREATCODE_DRIFTCTL_PATH: path.join(
          getFixturePath('iac'),
  ...
  describe('Test html file output', () => {
    ...
    it('Launch driftctl with html output format', async () => {
      const { stdout, stderr, exitCode } = await run(
        `threatcode iac describe --all --html-file-output=${htmlFile}`,
        {
          THREATCODE_FIXTURE_OUTPUT_PATH: outputFile,
          THREATCODE_DRIFTCTL_PATH: path.join(
            getFixturePath('iac'),
  ...
  it('Download and launch driftctl when executable is not found and org has the entitlement', async () => {
    ...
    const { stderr, exitCode } = await run(`threatcode iac describe --all`, {
      THREATCODE_DRIFTCTL_URL: apiUrl + '/download/driftctl',
      THREATCODE_CACHE_PATH: cachedir,
    });
...
describe('processDriftctlOutput', () => {
  it('test that html in stdout is parsed correctly', async () => {
    ...
    const expectedOutputData = fs.readFileSync(
      path.join(getFixturePath('iac'), 'drift', 'output', 'threatcode_output.html'),
    );
  ...
  it('test that html in file is parsed correctly', async () => {
    ...
    const tmpFilepath = '/tmp/threatcode-html-test.html';
