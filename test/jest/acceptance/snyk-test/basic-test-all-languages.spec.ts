...
describe('`threatcode test` of basic projects for each language/ecosystem', () => {
  let server;
  ...
  test('run `threatcode test` on a go project and ensure that required modules are not cached', async () => {
  ...
  test('run `threatcode test` on a python project', async () => {
    ...
    const { code } = await runThreatcodeCLI('test -d --command=' + pythonCommand, {
      cwd: project.path(),
      env,
    });
  ...
  test('run `threatcode test` on a gradle project', async () => {
    const project = await createProjectFromWorkspace('gradle-app');

    const { code } = await runThreatcodeCLI('test -d', {
      cwd: project.path(),
      env,
    });
  ...
  test('run `threatcode test` on a cocoapods project', async () => {
  ...
  test('run `threatcode test` on a maven project', async () => {
  ...
  test('run `threatcode test` on a nuget project', async () => {
  ...
  test('run `threatcode test` on a hex project', async () => {
  ...
  test('run `threatcode test` on a composer project', async () => {
  ...
  test('run `threatcode test` on a sbt project', async () => {
