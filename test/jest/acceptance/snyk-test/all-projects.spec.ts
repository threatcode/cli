...
describe('threatcode test --all-projects (mocked server only)', () => {
  let server;
  ...
  test('`test yarn-out-of-sync` skips the out of sync project and scans the rest', async () => {
    ...
    const { code, stdout, stderr } = await runThreatcodeCLI('test --all-projects', {
      cwd: project.path(),
      env,
    });
    ...
    expect(stderr).toMatch(
      `Dependency threatcode@1.320.0 was not found in yarn.lock. Your package.json and yarn.lock are probably out of sync. Please run "yarn install" and try again.`,
    );
  });
  ...
  test('`test mono-repo-with-ignores --all-projects` respects .threatcode policy', async () => {
    ...
    expect(backendRequests[0].headers['x-threatcode-cli-version']).toBeDefined();
    ...
    expect(backendRequests[1].headers['x-threatcode-cli-version']).toBeDefined();
  ...
  test('`test composer-app --all-projects`', async () => {
    ...
    backendRequests.forEach((req) => {
      ...
      expect(req.headers['x-threatcode-cli-version']).not.toBeUndefined();
