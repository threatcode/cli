...
import { startThreatcodeCLI, TestCLI } from '../../util/startThreatcodeCLI';

describe('threatcode-apps: create app', () => {
  ...
  beforeAll((done) => {
    ...
    env = {
      ...
      THREATCODE_API_REST_URL: 'http://localhost:' + port + baseApi,
  ...
  describe('experimental flag', () => {
    it('should throw error without the experimental flag', async () => {
      cli = await startThreatcodeCLI('apps create');
  ...
  describe('help', () => {
    it('should print the apps helps document when apps subcommand invalid', async () => {
      cli = await startThreatcodeCLI('apps invalid');
      // Check for first line to confirm help docs were indeed printed
      await expect(cli).toDisplay(
        'Threatcode Apps are integrations that extend the functionality of the Threatcode platform',
      );
    });
  });
  ...
  describe('interactive mode', () => {
    it('should prompt for app name and print error if not provided', async () => {
      cli = await startThreatcodeCLI('apps create --interactive --experimental', {
        env,
      });
      // Assert for first question that is the name and error when no name provided
      await expect(cli).toDisplay(
        'Name of the Threatcode App (visible to users when they install the Threatcode App)?',
      );
    ...
    it('should prompt for redirect uris and print error if not provided', async () => {
      ...
      await expect(cli).toDisplay(
        "Your Threatcode App's redirect URIs (comma seprated list.",
      );
    ...
    it('should prompt for scopes and print error if not provided', async () => {
      ...
      // Assert
      await expect(cli).toDisplay("Your Threatcode App's permission scopes");
      await cli.answer('');
      await expect(cli).toDisplay('Please enter something');
    });
    ...
    it('should create app with user provided data (interactive mode)', async () => {
      ...
      // Assert
      await expect(cli).toDisplay('Threatcode App created successfully!');
    ...
    it('should prompt users for input with flag shortcut (-i)', async () => {
      cli = await startThreatcodeCLI('apps create -i --experimental', {
        env,
      });
  ...
  describe('scriptable mode', () => {
    ...
    it('should throw error when app name not provided', async () => {
      const cli = await startThreatcodeCLI(
    ...
    it('throws an error when an invalid context is provided', async () => {
      ...
      await expect(cli).toDisplay(
        "Option '--context' must be either 'tenant' or 'user'! For interactive mode, please use '--interactive' or '-i' flag. For more information please run the help command 'threatcode apps --help' or 'threatcode apps -h'.",
      );
      await expect(cli).toExitWith(2);
    });
