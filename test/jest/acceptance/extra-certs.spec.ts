...
describe('Extra CA certificates specified with `NODE_EXTRA_CA_CERTS`', () => {
  it('using a not existing file', async () => {
    const { code } = await runThreatcodeCLI(`woof --debug`, {
      env: {
        ...process.env,
  ...
  it('using a valid cert file', async () => {
    ...
    const res = await runCommand(
      ...
      { cwd: 'cliv2', env: { ...process.env, THREATCODE_DNS_NAMES: 'localhost' } },
    );
    ...
    const THREATCODE_API = 'https://localhost:' + port + baseApi;
    ...
    const res1 = await runThreatcodeCLI(`test --debug`, {
      env: {
        ...process.env,
    ...
    const res2 = await runThreatcodeCLI(`test --debug`, {
      env: {
        ...process.env,
