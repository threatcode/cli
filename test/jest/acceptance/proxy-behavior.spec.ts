...
const THREATCODE_API_HTTPS = 'https://threatcode.io/api/v1';
const THREATCODE_API_HTTP = 'http://threatcode.io/api/v1';
...
describe('Proxy Authentication (all platforms)', () => {
  if (canTestRun()) {
    ...
    it('successfully runs threatcode test with proxy (Basic)', async () => {
      ...
      const additionalLogEntries = logOnExit.substring(logOnEntry.length);
      expect(additionalLogEntries.includes('TCP_TUNNEL/200')).toBeTruthy();
      expect(additionalLogEntries.includes('CONNECT threatcode.io:443')).toBeTruthy();
    });
  }
});
