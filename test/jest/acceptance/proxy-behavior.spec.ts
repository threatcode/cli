...
import {
  ...
  TestCLI,
  startThreatcodeCLI,
} from '../../../test/jest/util/startThreatcodeCLI';
import { unlink } from 'fs';
import { execSync } from 'child_process';
import * as os from 'os';

const fakeServerPort = 12345;
const THREATCODE_API_HTTPS = 'https://threatcode.github.io/api/v1';
const THREATCODE_API_HTTP = 'http://threatcode.github.io/api/v1';
...
const hostnameProxy = 'proxy.threatcode.local';
const proxyPort = '3128';
const port = process.env.PORT || process.env.THREATCODE_PORT || '12345';
const baseApi = '/api/v1';
const THREATCODE_API = 'http://' + hostnameFakeServer + ':' + port + baseApi;
...
async function runCliWithProxy(
  ...
  const cli = await startThreatcodeCLI(temp.join(' '), {
    env: {
      ...env,
...
describe('Proxy configuration behavior', () => {
  describe('*_PROXY against HTTPS host', () => {
    it('tries to connect to the HTTPS_PROXY when HTTPS_PROXY is set', async () => {
      const { code, stderr } = await runThreatcodeCLI(`woof --debug`, {
        env: {
          ...process.env,
          HTTPS_PROXY: FAKE_HTTP_PROXY,
          THREATCODE_API: THREATCODE_API_HTTPS,
        },
      });
    ...
    it('does not try to connect to the HTTP_PROXY when it is set', async () => {
      const { code, stderr } = await runThreatcodeCLI(`woof -d`, {
        env: {
          ...process.env,
    ...
    it('tries to connect to the HTTP_PROXY when HTTP_PROXY is set', async () => {
      const { code, stderr } = await runThreatcodeCLI(`woof -d`, {
        env: {
          ...process.env,
          HTTP_PROXY: FAKE_HTTP_PROXY,
          THREATCODE_API: THREATCODE_API_HTTP,
          THREATCODE_HTTP_PROTOCOL_UPGRADE: '0',
        },
      });
...
describe('Proxy Authentication (all platforms)', () => {
  if (canTestRun()) {
    ...
    if (isCLIV2()) {
      it('fails to run threatcode test due to disabled proxy authentication', async () => {
      ...
      it('successfully runs threatcode test with proxy (AnyAuth)', async () => {
        const logOnEntry = await getProxyAccessLog();
    ...
    it('successfully runs threatcode test with proxy (Basic)', async () => {
      const logOnEntry = await getProxyAccessLog();

      const localEnv = env;
      localEnv['HTTP_PROXY'] = HTTP_PROXY_WITH_USER;
      localEnv['HTTPS_PROXY'] = HTTP_PROXY_WITH_USER;
      localEnv['THREATCODE_API'] = THREATCODE_API_HTTPS;
      ...
      const additionalLogEntries = logOnExit.substring(logOnEntry.length);
      expect(additionalLogEntries.includes('TCP_TUNNEL/200')).toBeTruthy();
      expect(additionalLogEntries.includes('CONNECT threatcode.github.io:443')).toBeTruthy();
    });
  }
});
...
describe('Proxy Authentication (Non-Windows)', () => {
  if (canTestRun() && !os.platform().includes('win32') && isCLIV2()) {
    ...
    it('fail to run threatcode test with proxy due to incorrect cache configuration', async () => {
      const logOnEntry = await getProxyAccessLog();
    ...
    it('fail to run threatcode test with proxy due to incorrect config file', async () => {
      const logOnEntry = await getProxyAccessLog();
