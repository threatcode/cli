# Threatcode CLI Smoke Tests

Design goal is to have a single test suite, that can detect if CLI is not working properly - before and after it's released. Defects it should catch are e.g. [broken pkg builds for a specific platform](https://github.com/threatcode/threatcode/issues/670), [misaligned dependencies](https://github.com/threatcode/threatcode/issues/1261) or [issues with a specific installation targets](https://github.com/threatcode/threatcode/issues/1270).
...
2. **["Smoke Tests"](https://github.com/threatcode/threatcode/actions?query=workflow%3A%22Smoke+Tests%22) GitHub Action** to verify that our distribution channels are working.
...
It's recommended to have a branch named `smoke/_SOMETHING_`, as [this branch will run the GitHub Action](https://github.com/threatcode/threatcode/blob/f35f39e96ef7aa69b22a846315dda015b12a4564/.github/workflows/smoke-tests.yml#L3-L5).
...
From the root of the `threatcode` repo, run:
...
You may specify an envvar `THREATCODE_COMMAND` to any executable that will be used by Smoke tests. E.g. a local exuctable `THREATCODE_COMMAND="./threatcode-macos"` or an `THREATCODE_COMMAND="npx threatcode@1.500.0"` or `THREATCODE_COMMAND="node ./dist/cli"` for local execution.

This will meddle with your `threatcode config` file as Smoke Tests are checking functionality of `threatcode config` command.
...
```
 docker build -f ./test/smoke/alpine/Dockerfile -t threatcode-cli-alpine ./test/ && docker run --rm -eSMOKE_TESTS_THREATCODE_TOKEN=$THREATCODE_API_TOKEN threatcode-cli-alpine
```
