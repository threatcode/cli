...
export const AllProjectsTests: AcceptanceTests = {
  language: 'Mixed',
  tests: {
    '`monitor mono-repo-with-ignores --all-projects` respects .threatcode policy': (
      params,
      utils,
    ) => async (t) => {
    ...
    '`monitor mono-repo-project --all-projects --detection-depth=1`': (
      ...
      const pluginsWithoutTargetFileInBody = [
        'threatcode-nodejs-lockfile-parser',
        'bundled:maven',
        'bundled:rubygems',
        'threatcode:sbt',
      ];
    ...
    '`monitor maven-multi-app --all-projects --detection-depth=2`': (
      ...
      requests.forEach((request) => {
        ...
        t.equal(
          request.headers['x-threatcode-cli-version'],
          params.versionNumber,
          'sends version number',
        );
      });
    ...
    '`monitor monorepo-bad-project --all-projects`': (params, utils) => async (
      ...
      t.match(
        result,
        'Dependency threatcode@* was not found in yarn.lock',
