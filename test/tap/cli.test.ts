...
process.env.THREATCODE_API = 'http://localhost:' + port + BASE_API;
process.env.THREATCODE_HOST = 'http://localhost:' + port;
...
test('auth via key', async (t) => {
  try {
    ...
    t.notEqual(res.toLowerCase().indexOf('ready'), -1, 'threatcode auth worked');
  } catch (e) {
    t.threw(e);
  }
});
...
test('auth with no args', async (t) => {
  ...
  try {
    ...
    t.match(
      res,
      'Your account has been authenticated. Threatcode is now ready to be used',
      'threatcode auth worked',
    );
...
test('auth with UTMs in environment variables', async (t) => {
  ...
  process.env.THREATCODE_UTM_MEDIUM = 'ide';
  process.env.THREATCODE_UTM_SOURCE = 'eclipse';
  process.env.THREATCODE_UTM_CAMPAIGN = 'plugin';
  ...
  try {
    ...
    delete process.env.THREATCODE_UTM_MEDIUM;
    delete process.env.THREATCODE_UTM_SOURCE;
    delete process.env.THREATCODE_UTM_CAMPAIGN;
    // clean up stubs
    ciStub.restore();
    dockerStub.restore();
...
test('threatcode ignore - all options', async (t) => {
...
test('threatcode ignore - no ID', async (t) => {
  try {
    const dir = await makeTmpDirectory();
    await cli.ignore({
      reason: 'REASON',
...
test('threatcode ignore - default options', async (t) => {
  const clock = sinon.useFakeTimers(new Date(2016, 11, 1).getTime());
  try {
    const dir = await makeTmpDirectory();
    await cli.ignore({
...
test('threatcode ignore - not authorized', async (t) => {
  const dir = await makeTmpDirectory();
...
test('threatcode policy', async (t) => {
  await cli.policy();
...
after('teardown', async (t) => {
  delete process.env.THREATCODE_API;
  delete process.env.THREATCODE_HOST;
  delete process.env.THREATCODE_PORT;
  t.notOk(process.env.THREATCODE_PORT, 'fake env values cleared');
