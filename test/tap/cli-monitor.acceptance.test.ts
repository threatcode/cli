...
import * as depGraphLib from '@threatcode/dep-graph';
import { getFixturePath } from '../jest/util/getFixturePath';
import { getWorkspacePath } from '../jest/util/getWorkspacePath';
import { threatcodeHttpClient } from '../../src/lib/request/threatcode-http-client';
...
if (!isWindows) {
  ...
  test('`monitor --policy-path`', async (tt) => {
    ...
    tt.test('default policy', async (t) => {
      ...
      const expected = fs.readFileSync(path.join('.threatcode'), 'utf8');
    ...
    tt.test('custom policy path', async (t) => {
      ...
      const expected = fs.readFileSync(
        path.join('custom-location', '.threatcode'),
        'utf8',
      );
      t.equal(policyString, expected, 'sends correct policy');
    });
  ...
  test('`monitor gradle --prune-repeated-subdependencies`', async (t) => {
    ...
    t.equal(
      ...
      'verify if the generated depGraph from threatcode monitor has been pruned',
    );
  });
    ...
    const plugin = {
      async inspect() {
        return {
          plugin: {
            targetFile: 'Podfile',
            name: 'threatcode-cocoapods-plugin',
            runtime: 'cocoapods',
          },
          package: {},
        };
