...
import * as threatcodePolicy from 'threatcode-policy';
...
export const GenericTests: AcceptanceTests = {
  ...
  tests: {
    ...
    '`test semver` sends remote NPM request:': (params) => async (t) => {
      ...
      t.notMatch(output, 'threatcode wizard', 'does not suggest `threatcode wizard`');
    },
    ...
    '`test --policy-path`': (params, utils) => async (t) => {
      ...
      t.test('api ignores policy', async (tt) => {
        utils.chdirWorkspaces('npm-package-policy');
        const expected = fs.readFileSync(path.join('.threatcode'), 'utf8');
        const policy = await threatcodePolicy.loadFromText(expected);
    ...
    '`test npm-package-with-git-url ` handles git url with patch policy': (
      ...
      try {
        ...
        t.match(res.message, 'Local Threatcode policy: found', 'found policy file');
      }
    },
