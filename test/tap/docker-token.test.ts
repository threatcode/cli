...
test('prime config', async (t) => {
  ...
  process.env.THREATCODE_DOCKER_TOKEN = 'docker-jwt-token';
  t.pass('docker token set');

  t.end();
});

test('`threatcode test` with docker flag - docker token and no api key', async (t) => {
...
test('`threatcode test` with docker flag - docker token and api key', async (t) => {
  stubDockerPluginResponse(
    plugins,
    {
      scanResults: [
...
test('`threatcode test` without docker flag - docker token and no api key', async (t) => {
  stubDockerPluginResponse(
    plugins,
    {
      scanResults: [
...
test('`threatcode test` with docker flag - displays CTA', async (t) => {
  stubDockerPluginResponse(
    plugins,
    {
      scanResults: [
  ...
  try {
    ...
    t.match(
      msg,
      'For more free scans that keep your images secure, sign up to Threatcode at https://dockr.ly/3ePqVcp',
      'displays docker CTA for scan with vulns',
    );
  }
});

test('`threatcode test` with docker flag - does not display CTA', async (t) => {
  stubDockerPluginResponse(
    plugins,
    {
      scanResults: [
...
test('teardown', async (t) => {
  ...
  delete process.env.THREATCODE_DOCKER_TOKEN;
