...
describe('Get Shasum', () => {
  it('Shasum available (multiple)', async () => {
    ...
    fs.writeFileSync(
      file,
      '098fe123  *threatcode-win\n12345  *threatcode-macos\ncecece  *threatcode-linux-arm64\n0a238fe123  *threatcode-linux',
    );

    const actual = common.getCurrentSha256sum('threatcode-linux', file);
    expect(actual).toEqual(expected);
  ...
  it('Shasum available (single)', async () => {
    const expected = '0a238fe123';
    const file = path.join(__dirname, 'sha256sums.txt' + Math.random());
    fs.writeFileSync(file, '0a238fe123  threatcode-linux\n');
  ...
  it('Shasum not available', async () => {
    ...
    const actual = common.getCurrentSha256sum('threatcode-linux-arm64', file);
...
describe('Testing binary bootstrapper', () => {
  it('downloadExecutable() succesfull', async () => {
    const binaryName = 'threatcode-macos';
