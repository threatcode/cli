...
import { CallGraph, ScannedProject } from '@threatcode/cli-interface/legacy/common';
  ...
  const { res, body } = await makeRequest({
    body: {
      meta: {
        ...
        id: threatcode.id || depTree.name,
        ci: isCI(),
        pid: process.pid,
        node: process.version,
        master: threatcode.config.isMaster,
  ...
  const { res, body } = await makeRequest({
    body: {
      meta: {
        method: meta.method,
        hostname: os.hostname(),
        id: threatcode.id || depGraph.rootPkg.name,
        ci: isCI(),
        pid: process.pid,
        node: process.version,
        master: threatcode.config.isMaster,
