...
import threatcodeLogoSVG from './assets/threatcode-logo';
import threatcodeFaviconBase64 from './assets/threatcode-favicon';
...
function rebrandHTMLOutput(data: string): string {
  ...
  data = data.replace(
    faviconReplaceRegex,
    `<link rel="shortcut icon" type="image/x-icon" href="${threatcodeFaviconBase64}" />`,
  );
  ...
  data = data.replace(
    titleReplaceRegex,
    `<title>Threatcode IaC drift report</title>`,
  );
  ...
  data = data.replace(
    logoReplaceRegex,
    `<div id="brand_logo">${threatcodeLogoSVG}</div>`,
  );
