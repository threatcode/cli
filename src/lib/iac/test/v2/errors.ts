...
const threatcodeIacTestErrorsUserMessages = {
  ...
  OpenBundle: "The Threatcode CLI couldn't open the rules bundle",
  ...
  NoLoadableInput:
    "The Threatcode CLI couldn't find any valid IaC configuration files to scan",
...
export function getErrorUserMessage(code: number, error: string): string {
  if (code < 2000 || code >= 3000) {
    return 'INVALID_THREATCODE_IAC_TEST_ERROR';
  }
  ...
  return threatcodeIacTestErrorsUserMessages[errorName];
}

export class ThreatcodeIacTestError extends CustomError {
