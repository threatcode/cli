import { SEVERITY } from '../../../../threatcode-test/common';
import { IacProjectType } from '../../../constants';
import { ThreatcodeIacTestError } from '../errors';
...
export function mapThreatcodeIacTestOutputToTestOutput(
  threatcodeIacOutput: ThreatcodeIacTestOutput,
): TestOutput {
  const entitlementError = threatcodeIacOutput.errors?.find(
  ...
  const readSettingsError = threatcodeIacOutput.errors?.find(
  ...
  const errors = threatcodeIacOutput.errors?.map((err) => new ThreatcodeIacTestError(err));
  ...
  return {
    results: threatcodeIacOutput.results,
    settings: threatcodeIacOutput.settings,
    errors,
  };
}
...
export interface TestOutput {
  results?: Results;
  errors?: ThreatcodeIacTestError[];
  settings: Settings;
}

export interface ThreatcodeIacTestOutput {
  results?: Results;
