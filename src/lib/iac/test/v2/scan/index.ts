...
import { mapThreatcodeIacTestOutputToTestOutput, TestOutput } from './results';
...
const debug = newDebug('threatcode-iac');

export const systemCachePath = config.CACHE_PATH ?? envPaths('threatcode').cache;
...
async function scanWithConfig(
  ...
  env['THREATCODE_IAC_TEST_API_REST_URL'] =
    process.env['THREATCODE_IAC_TEST_API_REST_URL'] || getApiUrl();
  env['THREATCODE_IAC_TEST_API_REST_TOKEN'] =
    process.env['THREATCODE_IAC_TEST_API_REST_TOKEN'] || getApiToken();
  env['THREATCODE_IAC_TEST_API_REST_OAUTH_TOKEN'] =
    process.env['THREATCODE_IAC_TEST_API_REST_OAUTH_TOKEN'] || getOAuthToken();
  env['THREATCODE_IAC_TEST_API_V1_URL'] =
    process.env['THREATCODE_IAC_TEST_API_V1_URL'] || getApiUrl();
  env['THREATCODE_IAC_TEST_API_V1_TOKEN'] =
    process.env['THREATCODE_IAC_TEST_API_V1_TOKEN'] || getApiToken();
  env['THREATCODE_IAC_TEST_API_V1_OAUTH_TOKEN'] =
    process.env['THREATCODE_IAC_TEST_API_V1_OAUTH_TOKEN'] || getOAuthToken();
  ...
  return mapThreatcodeIacTestOutputToTestOutput(await readJson(outputPath));
}
...
function processFlags(
  ...
  if (options.threatcodeCloudEnvironment) {
    flags.push('-threatcode-cloud-environment', options.threatcodeCloudEnvironment);
  }
...
async function createTemporaryFiles(
  ...
  try {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), 'threatcode-'));
    const tempOutput = path.join(tempDir, 'output.json');
    const tempPolicy = path.join(tempDir, '.threatcode');
