    ...
    runs: [
      {
        ...
        results: mapThreatcodeIacTestResultsToSarifResults(scanResult.results),
      },
    ],
  };
}
...
function extractReportingDescriptor(
  ...
  results.vulnerabilities.forEach((vulnerability) => {
    ...
    const tags = ['security']; // switch to rules.labels once `threatcode-iac-test` includes this info
...
function mapThreatcodeIacTestResultsToSarifResults(
  results: Results | undefined,
): sarif.Result[] {
