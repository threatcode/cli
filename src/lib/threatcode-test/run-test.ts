...
import { parsePackageString as moduleToObject } from 'threatcode-module';
...
const debug = debugModule('threatcode:run-test');
...
async function parseRes(
  ...
  if (!options['ignore-policy']) {
    ...
    const policy = await threatcode.policy.loadFromText(res.policy);
...
async function assembleRemotePayloads(root, options): Promise<Payload[]> {
  ...
  // options.vulnEndpoint is only used by `threatcode protect` (i.e. local filesystem tests)
