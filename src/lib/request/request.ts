...
const debug = debugModule('threatcode:req');
const threatcodeDebug = debugModule('threatcode');
...
function setupRequest(payload: Payload) {
  ...
  payload.headers['x-threatcode-cli-version'] = versionNumber;
  ...
  if (body && !noCompression) {
    ...
    threatcodeDebug('sending request to:', payload.url);
    threatcodeDebug('request body size:', json.length);
    threatcodeDebug('gzipped request body size:', data.length);

    let callGraphLength: number | null = null;
    if (body.callGraph) {
      callGraphLength = JSON.stringify(body.callGraph).length;
      threatcodeDebug('call graph size:', callGraphLength);
    }
  ...
  if (
    ...
    process.env.THREATCODE_HTTP_PROTOCOL_UPGRADE !== '0'
  ) {
  ...
  if (proxyUri) {
    threatcodeDebug('using proxy:', proxyUri);
    bootstrap({
      environmentVariableNamespace: '',
    });
  } else {
    threatcodeDebug('not using proxy');
  }
