...
const { SEVERITIES } = require('../threatcode-test/common');
const { errorMessageWithRetry } = require('./error-with-retry');
const analytics = require('../analytics');
const { FormattedCustomError } = require('./formatted-custom-error');

const errors = {
  connect: 'Check your network connection, failed to connect to Threatcode API',
  endpoint: 'The Threatcode API is not available on ' + config.API,
  auth: 'Unauthorized: please ensure you are logged in using `threatcode auth`',
  oldthreatcode:
    'You have an alpha format Threatcode policy file in this directory. ' +
    'Please remove it.',
  ...
  nodeModules:
    ...
    're-run your threatcode command.',
  noApiToken:
    '%s requires an authenticated account. Please run `threatcode auth` ' +
    'and try again.',
  ...
  policyFile:
    ...
    'directory with a .threatcode file',
  idRequired: 'id is a required field for `threatcode ignore`',
  unknownCommand: '%s\n\nRun `threatcode --help` for a list of available commands.',
...
const codes = {
  ...
  OLD_DOTFILE_FORMAT: errors.oldthreatcode,
