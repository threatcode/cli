...
import {
  FeatureNotSupportedByThreatcodeCodeError,
...
const debug = debugLib('threatcode-code');
...
function severityToAnalysisSeverity(severity: SEVERITY): AnalysisSeverity {
  if (severity === SEVERITY.CRITICAL) {
    throw new FeatureNotSupportedByThreatcodeCodeError(SEVERITY.CRITICAL);
  }
  const severityLevel = {
    low: 1,
    medium: 2,
