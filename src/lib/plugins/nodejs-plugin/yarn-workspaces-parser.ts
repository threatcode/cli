...
const debug = baseDebug('threatcode-yarn-workspaces');
import * as lockFileParser from 'threatcode-nodejs-lockfile-parser';
import {
...
export async function processYarnWorkspaces(
  ...
  const result: MultiProjectResultCustom = {
    plugin: {
      name: 'threatcode-nodejs-yarn-workspaces',
  ...
  for (const directory of Object.keys(yarnTargetFiles)) {
    ...
    try {
      ...
      const project: ScannedProjectCustom = {
        ...
        depTree: res as any,
        plugin: {
          name: 'threatcode-nodejs-lockfile-parser',
          runtime: process.version,
        },
      };
