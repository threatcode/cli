...
import { parsePackageString as threatcodeModule } from 'threatcode-module';
import {
...
import {
  ...
  DockerIssue,
  SEVERITY,
} from '../../lib/threatcode-test/legacy';
import { formatLegalInstructions } from './legal-license-instructions';
import { colorTextBySeverity } from '../../lib/threatcode-test/common';
    ...
    const upgradePathsArray = uniq(
      vuln.list.map((v) => {
        ...
        if (shouldUpgradeItself) {
          ...
          // Example command: threatcode test express@3
          const selfUpgradeInfo =
            v.upgradePath.length > 0
              ? ` (triggers upgrades to ${v.upgradePath.join(PATH_SEPARATOR)})`
              : '';
          const testedPackageName = threatcodeModule(v.upgradePath[0] as string);
          return (
