...
import { IacTestResponse } from '../../lib/threatcode-test/iac-test-result';
...
export function formatTestMeta(
  ...
  if (res.payloadType !== 'k8sconfig') {
    ...
    if (legacyRes.filesystemPolicy) {
      meta.push(
        chalk.bold(rightPadWithSpaces('Local Threatcode policy: ', padToLength)) +
          chalk.green('found'),
      );
      if (
        ...
        meta.push(
          chalk.bold(
            rightPadWithSpaces('Local Threatcode policy ignored: ', padToLength),
          ) + chalk.red('yes'),
        );
      }
    }
