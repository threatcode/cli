...
} from 'threatcode-docker-plugin/dist';
...
export function extractDataToSendFromResults(
  ...
  return {
    ...
    stringifiedData, // this will be used to display either the Threatcode or SARIF format JSON to stdout if --json or --sarif is set
...
function getDockerSuggestionText(
  ...
  if (!options.file && !result?.docker?.baseImage) {
    tips.push(
      `Threatcode wasn’t able to auto detect the base image, use \`--file\` option to get base image remediation advice.
Example: $ threatcode container test ${options.path} --file=path/to/Dockerfile`,
    );
  }
  ...
  if (options.docker && result.targetFile && result.uniqueCount > 0) {
    tips.push(
      'Threatcode found some vulnerabilities in your image applications (Threatcode searches for these vulnerabilities by default). See https://threatcode.co/app-vulns for more information.',
    );
  }
  ...
  return (
    ...
    tips
      ...
      .concat(
        ...
        } in the future, please run \`threatcode config set disableSuggestions=true\``,
      )
      .join('\n\n')
  );
}
