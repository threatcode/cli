...
import { monitor as threatcodeMonitor } from '../../../lib/monitor';
import { processJsonMonitorResponse } from './process-json-monitor';
import threatcode = require('../../../lib'); // TODO(kyegupov): fix import
...
import { PluginMetadata } from '@threatcode/cli-interface/legacy/plugin';
  ...
  for (const path of paths) {
    ...
    try {
      ...
      if (unsupportedPackageManager) {
        return `${unsupportedPackageManager.label} projects do not currently support "threatcode monitor"`;
      }
      const targetFile =
        !options.scanAllUnmanaged && options.docker && !options.file // threatcode monitor --docker (without --file)
          ? undefined
...
function validateMonitorPath(path: string, isDocker?: boolean): void {
  ...
  if (!exists && !isDocker) {
    throw new Error('"' + path + '" is not a valid path for "threatcode monitor"');
  }
}
