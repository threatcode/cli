...
const debug = Debug('threatcode-auth');
let attemptsLeft = 0;
...
export default async function auth(apiToken: string): Promise<string> {
  ...
  return promise.then((data) => {
    ...
    if (res.statusCode === 200 || res.statusCode === 201) {
      threatcode.config.set('api', body.api);
      return (
        '\nYour account has been authenticated. Threatcode is now ready to ' +
        'be used.\n'
      );
    }
...
function browserAuthPrompt(isDocker: boolean, urlStr: string): string {
  if (isDocker) {
    ...
    return (
      ...
      "be ready to start using threatcode.\n\nIf you can't wait use this url:\n" +
      urlStr +
      '\n'
    );
  }
