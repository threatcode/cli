...
import { LegacyVulnApiResult, TestResult } from '../../../lib/threatcode-test/legacy';
...
import { FailOn } from '../../../lib/threatcode-test/common';
import {
...
import {
  getPackageJsonPathsContainingThreatcodeDependency,
...
const debug = Debug('threatcode-test');
const SEPARATOR = '\n-------------------------------------------------------\n';
...
export default async function test(
  ...
  const packageJsonPathsWithThreatcodeDepForProtect: string[] = getPackageJsonPathsContainingThreatcodeDependency(
    options.file,
    paths,
  );

  analytics.add(
    'upgradable-threatcode-protect-paths',
    packageJsonPathsWithThreatcodeDepForProtect.length,
  );
  ...
  for (const path of paths) {
    ...
    try {
      res = await threatcode.test(path, testOpts);
    } catch (error) {
    ...
    if (options.failOn) {
      ...
      if (!fail) {
        ...
        response += getProtectUpgradeWarningForPaths(
          packageJsonPathsWithThreatcodeDepForProtect,
        );
