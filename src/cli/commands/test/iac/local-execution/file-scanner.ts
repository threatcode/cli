...
import { SEVERITIES } from '../../../../../lib/threatcode-test/common';
...
export function validateResultFromCustomRules(
  ...
  for (const violatedPolicy of result.violatedPolicies) {
    ...
    const invalidThreatcodePublicId = violatedPolicy.publicId.startsWith('THREATCODE-CC-');
    if (invalidThreatcodePublicId) {
      failureReason = `Invalid publicId for custom rule ${violatedPolicy.publicId}. Change to a publicId that does not start with THREATCODE-CC-`;
    }
