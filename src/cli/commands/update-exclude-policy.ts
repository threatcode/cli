...
export default async (...args: MethodArgs): Promise<any> => {
  ...
  // Ensure that this update-exclude-policy command can only be runned when using `threatcode iac update-exclude-policy`
  // Avoid `threatcode update-exclude-policy` direct usage
  if (options.iac != true) {
  ...
  try {
    ...
    let policy: Policy;
    try {
      policy = await threatcodePolicyLib.load();
    } catch (error) {
      if (error.code === 'ENOENT') {
        // policy file does not exist - create it
        policy = await threatcodePolicyLib.create();
      } else {
        throw error;
      }
    }
    await updateExcludeInPolicy(policy, analysis, options);
    await threatcodePolicyLib.save(policy);
  } catch (e) {
