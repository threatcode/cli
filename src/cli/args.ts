...
const DEBUG_DEFAULT_NAMESPACES = [
  ...
  'threatcode-code',
  'threatcode-iac',
  'threatcode:find-files',
  'threatcode:run-test',
  'threatcode:prune',
  'threatcode-nodejs-plugin',
  'threatcode-gradle-plugin',
  'threatcode-sbt-plugin',
  'threatcode-mvn-plugin',
  'threatcode-yarn-workspaces',
];
...
export interface ArgsOptions {
  ...
  // (see the threatcode-mvn-plugin or threatcode-gradle-plugin)
...
export function args(rawArgv: string[]): Args {
  ...
  const debug = debugModule('threatcode');
  ...
  // an argument to our command, like `threatcode test package2test`
  ...
  // threatcode [mode?] [command] [paths?] [options-double-dash]
  ...
  // alias `-v` to `threatcode version`
  if (argv.version) {
    command = 'version';
  }

  // alias `--about` to `threatcode about`
  if (argv.about) {
    command = 'about';
  }

  if (!command || argv.help || command === 'help') {
    // bit of a song and dance to support `threatcode -h` and `threatcode help`
    ...
    // so that no argument gets deleted or ignored. This ensures `threatcode --help [command]` and `threatcode [command] --help` return the
    // specific help page instead of the generic one.
    // This change also covers the scenario of 'threatcode [mode] [command] --help' and 'threatcode --help [mode] [command]`.
    if (!argv._.length) {
  ...
  if (command && command.indexOf('config:') === 0) {
    ...
    // threatcode.config('set', 'api=x')
    // threatcode.config('get', 'api') // etc
