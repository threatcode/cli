...
import (
	...
	"github.com/threatcode/cli-extension-sbom/pkg/sbom"
	"github.com/threatcode/cli/cliv2/internal/cliv2"
	"github.com/threatcode/cli/cliv2/internal/constants"
	"github.com/threatcode/cli/cliv2/pkg/basic_workflows"
	"github.com/threatcode/go-application-framework/pkg/analytics"
	"github.com/threatcode/go-application-framework/pkg/app"
	"github.com/threatcode/go-application-framework/pkg/auth"
	"github.com/threatcode/go-application-framework/pkg/configuration"
	localworkflows "github.com/threatcode/go-application-framework/pkg/local_workflows"
	"github.com/threatcode/go-application-framework/pkg/networking"
	"github.com/threatcode/go-application-framework/pkg/workflow"
	"github.com/threatcode/go-httpauth/pkg/httpauth"
	"github.com/threatcode/threatcode-iac-capture/pkg/capture"
...
func createCommandsForWorkflows(rootCommand *cobra.Command, engine workflow.Engine) {
	...
	for i := range workflowIdList {
		...
		// last parentCommand is the last level of the command, e.g. in the threatcode iac capture workflow, it would be 'capture'
...
func MainWithErrorCode() int {
	...
	if err != nil {
		...
		return constants.THREATCODE_EXIT_CODE_ERROR
	}
	...
	networkAccess.AddHeaderField("x-threatcode-cli-version", cliv2.GetFullVersion())
	networkAccess.AddHeaderField("User-Agent", "threatcode-cli/"+cliv2.GetFullVersion())
