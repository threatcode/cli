...
import (
	...
	pkg_utils "github.com/threatcode/go-application-framework/pkg/utils"
...
func legacycliWorkflow(
	...
	if oauthIsAvailable {
		...
		if oauth := config.GetString(auth.CONFIG_KEY_OAUTH_TOKEN); len(oauth) > 0 {
			...
			if _, ok := envMap[constants.THREATCODE_OAUTH_ACCESS_TOKEN_ENV]; !ok {
				env := []string{constants.THREATCODE_OAUTH_ACCESS_TOKEN_ENV + "=randomtoken"}
