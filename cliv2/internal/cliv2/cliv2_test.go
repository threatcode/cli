...
import (
	...
	"github.com/threatcode/cli/cliv2/internal/cliv2"
...
func getCacheDir(t *testing.T) string {
	t.Helper()
	cacheDir := path.Join(t.TempDir(), "threatcode")
...
func Test_PrepareV1EnvironmentVariables_Fill_and_Filter(t *testing.T) {
	...
	expected := []string{"something=1",
		"in=2",
		"here=3=2",
		"THREATCODE_INTEGRATION_NAME=foo",
		"THREATCODE_INTEGRATION_VERSION=bar",
		"HTTP_PROXY=proxy",
		"HTTPS_PROXY=proxy",
		"NODE_EXTRA_CA_CERTS=cacertlocation",
		"THREATCODE_SYSTEM_NO_PROXY=noProxy",
		"THREATCODE_SYSTEM_HTTP_PROXY=httpProxy",
		"THREATCODE_SYSTEM_HTTPS_PROXY=httpsProxy",
		"NO_PROXY=" + constants.THREATCODE_INTERNAL_NO_PROXY + ",noProxy",
	}
...
func Test_PrepareV1EnvironmentVariables_DontOverrideExistingIntegration(t *testing.T) {
	...
	input := []string{"something=1", "in=2", "here=3", "THREATCODE_INTEGRATION_NAME=exists", "THREATCODE_INTEGRATION_VERSION=already"}
	expected := []string{
		"something=1",
		"in=2",
		"here=3",
		"THREATCODE_INTEGRATION_NAME=exists",
		"THREATCODE_INTEGRATION_VERSION=already",
		"HTTP_PROXY=proxy",
		"HTTPS_PROXY=proxy",
		"NODE_EXTRA_CA_CERTS=cacertlocation",
		"THREATCODE_SYSTEM_NO_PROXY=",
		"THREATCODE_SYSTEM_HTTP_PROXY=",
		"THREATCODE_SYSTEM_HTTPS_PROXY=",
		"NO_PROXY=" + constants.THREATCODE_INTERNAL_NO_PROXY,
	}
...
func Test_PrepareV1EnvironmentVariables_OverrideProxyAndCerts(t *testing.T) {
	...
	expected := []string{
		...
		"HTTPS_PROXY=proxy",
		"NODE_EXTRA_CA_CERTS=cacertlocation",
		"THREATCODE_SYSTEM_NO_PROXY=312123",
		"THREATCODE_SYSTEM_HTTP_PROXY=exists",
		"THREATCODE_SYSTEM_HTTPS_PROXY=already",
		"NO_PROXY=" + constants.THREATCODE_INTERNAL_NO_PROXY + ",312123",
	}
...
func Test_PrepareV1EnvironmentVariables_Fail_DontOverrideExisting(t *testing.T) {
	...
	input := []string{"something=1", "in=2", "here=3", "THREATCODE_INTEGRATION_NAME=exists"}
	expected := input
...
func Test_prepareV1Command(t *testing.T) {
	...
	threatcodeCmd, err := cli.PrepareV1Command(
		"someExecutable",
		expectedArgs,
		getProxyInfoForTest(),
		"name",
	...
	assert.Contains(t, threatcodeCmd.Env, "THREATCODE_INTEGRATION_NAME=name")
	assert.Contains(t, threatcodeCmd.Env, "THREATCODE_INTEGRATION_VERSION=version")
	assert.Contains(t, threatcodeCmd.Env, "HTTPS_PROXY=http://threatcodecli:foo@127.0.0.1:1000")
	assert.Contains(t, threatcodeCmd.Env, "NODE_EXTRA_CA_CERTS=certLocation")
	assert.Equal(t, expectedArgs, threatcodeCmd.Args[1:])
	assert.Nil(t, err)
}
...
func Test_executeEnvironmentError(t *testing.T) {
	...
	_ = os.Setenv(constants.THREATCODE_INTEGRATION_NAME_ENV, "someName")
...
func Test_executeUnknownCommand(t *testing.T) {
	expectedReturnCode := constants.THREATCODE_EXIT_CODE_ERROR
