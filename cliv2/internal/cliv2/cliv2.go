...
import (
	...
	"github.com/threatcode/cli/cliv2/internal/constants"
	"github.com/threatcode/cli/cliv2/internal/embedded"
	"github.com/threatcode/cli/cliv2/internal/embedded/cliv1"
	"github.com/threatcode/cli/cliv2/internal/proxy"
	local_utils "github.com/threatcode/cli/cliv2/internal/utils"
	"github.com/threatcode/go-application-framework/pkg/utils"
)
...
func (c *CLI) GetIntegrationName() string {
	return constants.THREATCODE_INTEGRATION_NAME
}
...
func PrepareV1EnvironmentVariables(
	...
	_, integrationNameExists := inputAsMap[constants.THREATCODE_INTEGRATION_NAME_ENV]
	_, integrationVersionExists := inputAsMap[constants.THREATCODE_INTEGRATION_VERSION_ENV]

	if !integrationNameExists && !integrationVersionExists {
		inputAsMap[constants.THREATCODE_INTEGRATION_NAME_ENV] = integrationName
		inputAsMap[constants.THREATCODE_INTEGRATION_VERSION_ENV] = integrationVersion
	} else if !(integrationNameExists && integrationVersionExists) {
		err = EnvironmentWarning{message: fmt.Sprintf("Partially defined environment, please ensure to provide both %s and %s together!", constants.THREATCODE_INTEGRATION_NAME_ENV, constants.THREATCODE_INTEGRATION_VERSION_ENV)}
	}

	// preserve original proxy settings
	inputAsMap[constants.THREATCODE_HTTPS_PROXY_ENV_SYSTEM], _ = utils.FindValueCaseInsensitive(inputAsMap, constants.THREATCODE_HTTPS_PROXY_ENV)
	inputAsMap[constants.THREATCODE_HTTP_PROXY_ENV_SYSTEM], _ = utils.FindValueCaseInsensitive(inputAsMap, constants.THREATCODE_HTTP_PROXY_ENV)
	inputAsMap[constants.THREATCODE_HTTP_NO_PROXY_ENV_SYSTEM], _ = utils.FindValueCaseInsensitive(inputAsMap, constants.THREATCODE_HTTP_NO_PROXY_ENV)
	...
	if err == nil {
		...
		blackList := []string{
			constants.THREATCODE_HTTPS_PROXY_ENV,
			constants.THREATCODE_HTTP_PROXY_ENV,
			constants.THREATCODE_CA_CERTIFICATE_LOCATION_ENV,
			constants.THREATCODE_HTTP_NO_PROXY_ENV,
			constants.THREATCODE_NPM_NO_PROXY_ENV,
			constants.THREATCODE_NPM_HTTPS_PROXY_ENV,
			constants.THREATCODE_NPM_HTTP_PROXY_ENV,
			constants.THREATCODE_NPM_PROXY_ENV,
			constants.THREATCODE_NPM_ALL_PROXY,
		}
		...
		inputAsMap[constants.THREATCODE_HTTPS_PROXY_ENV] = proxyAddress
		inputAsMap[constants.THREATCODE_HTTP_PROXY_ENV] = proxyAddress
		inputAsMap[constants.THREATCODE_CA_CERTIFICATE_LOCATION_ENV] = caCertificateLocation

		// merge user defined (external) and internal no_proxy configuration
		if len(inputAsMap[constants.THREATCODE_HTTP_NO_PROXY_ENV_SYSTEM]) > 0 {
			internalNoProxy := strings.Split(constants.THREATCODE_INTERNAL_NO_PROXY, ",")
			externalNoProxy := regexp.MustCompile("[,;]").Split(inputAsMap[constants.THREATCODE_HTTP_NO_PROXY_ENV_SYSTEM], -1)
			mergedNoProxy := utils.Merge(internalNoProxy, externalNoProxy)
			inputAsMap[constants.THREATCODE_HTTP_NO_PROXY_ENV] = strings.Join(mergedNoProxy, ",")
		} else {
			inputAsMap[constants.THREATCODE_HTTP_NO_PROXY_ENV] = constants.THREATCODE_INTERNAL_NO_PROXY
		}
...
func (c *CLI) PrepareV1Command(
	...
	integrationName string,
	integrationVersion string,
) (threatcodeCmd *exec.Cmd, err error) {
	proxyAddress := fmt.Sprintf("http://%s:%s@127.0.0.1:%d", proxy.PROXY_USERNAME, proxyInfo.Password, proxyInfo.Port)

	threatcodeCmd = exec.Command(cmd, args...)
	threatcodeCmd.Env, err = PrepareV1EnvironmentVariables(c.env, integrationName, integrationVersion, proxyAddress, proxyInfo.CertificateLocation)

	return threatcodeCmd, err
}

func (c *CLI) executeV1Default(proxyInfo *proxy.ProxyInfo, passThroughArgs []string) error {
	threatcodeCmd, err := c.PrepareV1Command(c.v1BinaryLocation, passThroughArgs, proxyInfo, c.GetIntegrationName(), GetFullVersion())
	...
	if c.DebugLogger.Writer() != io.Discard {
		...
		variablesMap := utils.ToKeyValueMap(threatcodeCmd.Env, "=")
		listedEnvironmentVariables := []string{
			constants.THREATCODE_CA_CERTIFICATE_LOCATION_ENV,
			constants.THREATCODE_HTTPS_PROXY_ENV,
			constants.THREATCODE_HTTP_PROXY_ENV,
			constants.THREATCODE_HTTP_NO_PROXY_ENV,
			constants.THREATCODE_HTTPS_PROXY_ENV_SYSTEM,
			constants.THREATCODE_HTTP_PROXY_ENV_SYSTEM,
			constants.THREATCODE_HTTP_NO_PROXY_ENV_SYSTEM,
		}
	...
	threatcodeCmd.Stdin = c.stdin
	threatcodeCmd.Stdout = c.stdout
	threatcodeCmd.Stderr = c.stderr
...
func DeriveExitCode(err error) int {
	returnCode := constants.THREATCODE_EXIT_CODE_OK
	...
	if err != nil {
		if exitError, ok := err.(*exec.ExitError); ok {
			...
			returnCode = constants.THREATCODE_EXIT_CODE_ERROR
		}
	}
