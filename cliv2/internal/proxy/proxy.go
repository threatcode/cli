...
const (
	PROXY_REALM    = "threatcodecli_realm"
	PROXY_USERNAME = "threatcodecli"
)
...
func NewWrapperProxy(insecureSkipVerify bool, cacheDirectory string, cliVersion string, debugLogger *log.Logger) (*WrapperProxy, error) {
	...
	certName := "threatcode-embedded-proxy"
	...
	certFile, err := os.CreateTemp(tmpDirectory, "threatcode-cli-cert-*.crt")
	if err != nil {
	...
	if extraCaCertFile, ok := os.LookupEnv(constants.THREATCODE_CA_CERTIFICATE_LOCATION_ENV); ok {
