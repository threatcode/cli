...
echo "${THREATCODE_CODE_SIGNING_PGP_PRIVATE}" \
    | base64 --decode \
    | gpg --import --batch --passphrase "${THREATCODE_CODE_SIGNING_GPG_PASSPHRASE}"

echo "Signing shasums file"
gpg \
    --clear-sign \
    --local-user=3676C4B8289C296E \
    --passphrase="${THREATCODE_CODE_SIGNING_GPG_PASSPHRASE}" \
