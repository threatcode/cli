...
echo "Attempting to run Smoke Tests locally. See file 'test/smoke/README.md' for details. This will drop your local 'threatcode config'!"

if [ -z "$THREATCODE_API_TOKEN" ]; then
  echo "You need to set 'THREATCODE_API_TOKEN' envvar." >&2
  exit 1
fi
...
THREATCODE_COMMAND="node ${PWD}/bin/threatcode" REGRESSION_TEST=1 SMOKE_TESTS_SKIP_TEST_THAT_OPENS_BROWSER=1 SMOKE_TESTS_THREATCODE_TOKEN=$THREATCODE_API_TOKEN shellspec --chdir test/smoke test/smoke/spec/threatcode_auth_spec.sh -f d
