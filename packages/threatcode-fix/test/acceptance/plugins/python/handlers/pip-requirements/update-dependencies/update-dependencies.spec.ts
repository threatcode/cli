...
describe('fix *req*.txt / *.txt Python projects', () => {
  ...
  it('fixes project with a -r option', async () => {
    ...
    // Act
    const result = await threatcodeFix.fix([entityToFix], {
      quiet: true,
      stripAnsi: true,
    });
    // Assert
  ...
  it('does not add extra new lines', async () => {
    ...
    const expectedManifest =
      'Django==2.0.1\ntransitive>=1.1.1 # not directly required, pinned by Threatcode to avoid a vulnerability';
  ...
  it('fixes multiple files via -r with the same name (some were already fixed)', async () => {
      ...
      issues: [
        {
          ...
          issueId: 'THREATCODE-1',
          fixInfo: {},
        },
        {
          pkgName: 'Jinja2@2.7.2',
          issueId: 'THREATCODE-2',
          fixInfo: {},
        },
      ],
      issuesData: {
    ...
    // Act
    const result = await threatcodeFix.fix(
    ...
    expect(
      result.fixSummary
        .replace(/\\/g, '/')
        .replace(/packages\/threatcode-fix\//g, ''),
    ).toMatchSnapshot();
  ...
  it('fixes multiple files via -c & -r with the same name (some were already fixed)', async () => {
    ...
    const writeFileSpy = jest.spyOn(entityToFix1.workspace, 'writeFile');
    // Act
    const result = await threatcodeFix.fix([entityToFix2, entityToFix1], {
      quiet: true,
      stripAnsi: true,
    });
    const requirements = fs.readFileSync(
    ...
    expect(result.results.python.succeeded[1].changes).toEqual([
      {
        success: true,
        issueIds: ['THREATCODE-1', 'THREATCODE-2', 'THREATCODE-2', 'THREATCODE-3'],
        userMessage: expect.stringContaining('requirements.txt'),
      },
    ]);
  });
