...
import * as threatcodeFix from '../../src';
...
describe('Threatcode fix', () => {
  ...
  it('Threatcode fix returns results for supported type', async () => {
    // Arrange
    ...
    // Act
    const res = await threatcodeFix.fix([projectTestResult], {
      quiet: true,
      stripAnsi: true,
    });
  ...
  it('Threatcode fix returns results for supported type in dryRun mode (no write)', async () => {
    // Arrange
    const projectTestResult = generateEntityToFix(
      'pip',
      'requirements.txt',
    ...
    const writeFileSpy = jest.spyOn(projectTestResult.workspace, 'writeFile');
    // Act
    await threatcodeFix.fix([projectTestResult], {
      quiet: true,
      dryRun: true,
    });
  ...
  it('Threatcode fix returns results for supported & unsupported type', async () => {
    // Arrange
    const projectTestResult = generateEntityToFix(
      'pip',
      'requirements.txt',
    ...
    // Act
    const res = await threatcodeFix.fix(
  ...
  it('Threatcode fix returns results as expected', async () => {
    // Arrange
  ...
  it('Threatcode fix returns results as expected when 1 fails to fix', async () => {
    // Arrange
    const txtProdProjectTestResult = generateEntityToFix(
      'pip',
      'prod.txt',
  ...
  it('Threatcode fix returns results as expected when remediation data is empty', async () => {
    // Arrange
    const txtProdProjectTestResult = generateEntityToFix(
      'pip',
      'prod.txt',
    ...
    // Act
    const res = await threatcodeFix.fix([txtProdProjectTestResult], {
      quiet: true,
      stripAnsi: true,
    });
    // Assert
...
describe('groupEntitiesPerScanType', () => {
  it('correctly groups related entities per handler type (pip)', () => {
    ...
    // Act
    const res = threatcodeFix.groupEntitiesPerScanType([
      txtProdProjectTestResult,
      txtDevProjectTestResult,
      pipfileProjectTestResult,
    ]);
...
describe('Error handling', () => {
  it('Threatcode fix returns error when called with unsupported type', async () => {
    // Arrange
    const projectTestResult = generateEntityToFix(
      'npm',
      'package.json',
  ...
  it('Threatcode fix returns error when manifest can not be parsed', async () => {
    // Arrange
    const projectTestResult = generateEntityToFix(
      'pip',
      'requirements.txt',
