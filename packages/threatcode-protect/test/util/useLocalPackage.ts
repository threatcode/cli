...
const debug = debuglog('@threatcode' + __filename);

/**
 * Modifies the given fixture's package.json to use a locally built
 * @threatcode/protect tarball instead of the one in NPM Registry.
 *
...
const useLocalPackage = async (projectPath: string) => {
  ...
  if (packageJson.dependencies) {
    packageJson.dependencies['@threatcode/protect'] = path.resolve(
      workspaceRoot,
      tarballName,
    );
  }
