...
describe('@threatcode/protect', () => {
  ...
  describe('applies patch(es)', () => {
    it('works for project with a single patchable module', async () => {
      ...
      expect(
        fse.existsSync(
          project.path(
            `node_modules/nyc/node_modules/lodash/lodash.js.threatcode-protect.flag`,
          ),
        ),
      ).toBe(true);
      ...
      expect(
        fse.existsSync(
          project.path(
            `node_modules/nyc/node_modules/lodash/.threatcode-THREATCODE-JS-LODASH-567746.flag`,
          ),
        ),
      ).toBe(true);

      expect(log).toHaveBeenCalledWith('Applied Threatcode patches.');
    ...
    it('works for project with multiple patchable modules', async () => {
      ...
      expect(
        fse.existsSync(
          project.path(`node_modules/lodash/lodash.js.threatcode-protect.flag`),
        ),
      ).toBe(true);
      ...
      expect(
        fse.existsSync(
          project.path(`node_modules/lodash/.threatcode-THREATCODE-JS-LODASH-567746.flag`),
        ),
      ).toBe(true);
    ...
    it('works when the vulnId in a .threatcode file is quoted', async () => {
      const log = jest.spyOn(global.console, 'log');
      const postJsonSpy = jest.spyOn(http, 'postJson');
  ...
  describe('does not apply any patches and does not fail', () => {
    // in this scenario .threatcode file has a vulnId which corresponds to the `lodash` package, but there are not instances of lodash in the node_modules
    ...
    it('for project with no .threatcode file', async () => {
      const postJsonSpy = jest.spyOn(http, 'postJson');
      const project = await createProject('no-threatcode-file');
      const log = jest.spyOn(global.console, 'log');

      await protect(project.path());

      expect(log).toHaveBeenCalledWith('No .threatcode file found.');
  ...
  describe('does not send analytics if analytics is disabled', () => {
    describe('via threatcode.json file', () => {
      let tempConfigFolder: string;
      let tempThreatcodeConfigFile: string;

      beforeAll(() => {
        tempConfigFolder = fse.mkdtempSync(
          path.resolve(os.tmpdir(), `threatcode-config-`),
        );
        tempThreatcodeConfigFile = path.resolve(tempConfigFolder, 'threatcode.json');
        process.env.THREATCODE_CONFIG_FILE = tempThreatcodeConfigFile;
      });

      afterAll(() => {
        delete process.env.THREATCODE_CONFIG_FILE;
        fse.removeSync(tempConfigFolder);
      });

      it('when disable-analytics equals "1" (as a string)', async () => {
        fse.writeFileSync(
          tempThreatcodeConfigFile,
    ...
    describe('via THREATCODE_DISABLE_ANALYTICS env var', () => {
      it('when THREATCODE_DISABLE_ANALYTICS is "true"', async () => {
        process.env.THREATCODE_DISABLE_ANALYTICS = 'true';
        try {
          const postJsonSpy = jest.spyOn(http, 'postJson');
          const sendAnalyticsSpy = jest.spyOn(analytics, 'sendAnalytics');
          const project = await createProject('no-matching-paths');
          ...
          delete process.env.THREATCODE_DISABLE_ANALYTICS;
          console.log(delete process.env.THREATCODE_DISABLE_ANALYTICS);
        }
      });

      it('when THREATCODE_DISABLE_ANALYTICS is "1"', async () => {
        process.env.THREATCODE_DISABLE_ANALYTICS = '1';
        try {
          const postJsonSpy = jest.spyOn(http, 'postJson');
          const sendAnalyticsSpy = jest.spyOn(analytics, 'sendAnalytics');
          const project = await createProject('no-matching-paths');
  ...
  describe('outputs correct help documentation', () => {
    it('when called with --help', async () => {
      ...
      const { code, stdout } = await runCommand(
        'node',
        [path.resolve(__dirname, '../../bin/threatcode-protect'), '--help'],
        {
          cwd: project.path(),
        },
      );
  ...
  describe('outputs correct version', () => {
    it('when called with --version', async () => {
      ...
      const { code, stdout } = await runCommand(
        'node',
        [path.resolve(__dirname, '../../bin/threatcode-protect'), '--version'],
        {
          cwd: project.path(),
        },
      );
  ...
  describe('does not attempt to apply patch if a flag file exists indicating the file has already been patched', () => {
    it('with `.orig` file from the old protect', async () => {
      ...
      expect(code).toBe(0);
      expect(stdout).toContain('Applied Threatcode patches.');
    ...
    it('with `.threatcode-protect.flag` file from the new protect', async () => {
      const pathToProtectBin = path.resolve(
        __dirname,
        '../../bin/threatcode-protect',
      );
