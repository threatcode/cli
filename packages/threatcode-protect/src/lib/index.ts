...
import { extractPatchMetadata } from './threatcode-file';
...
async function protect(projectFolderPath: string) {
  ...
  const threatcodeFilePath = path.resolve(projectFolderPath, '.threatcode');

  if (!fs.existsSync(threatcodeFilePath)) {
    console.log('No .threatcode file found.');
    sendAnalytics({
      type: ProtectResultType.NO_THREATCODE_FILE,
    });
    return;
  }

  const threatcodeFileContents = fs.readFileSync(threatcodeFilePath, 'utf8');
  const threatcodeFilePatchMetadata = extractPatchMetadata(threatcodeFileContents);
  const vulnIdAndPackageNames: VulnIdAndPackageName[] = threatcodeFilePatchMetadata;
  const targetPackageNames = [
    ...new Set(threatcodeFilePatchMetadata.map((vpn) => vpn.packageName)), // get a list of unique package names by converting to Set and then back to array
  ];
  ...
  console.log('Applied Threatcode patches.');
